...
# Can only test ATB API using personal and secret API token.
# Fetch as environmental variable
ATB_API_TOKEN = os.getenv('ATB_API_TOKEN', None)
SETTINGS['atb_api_token'] = ATB_API_TOKEN
...
class TestAPI(unittest.TestCase):
    ...
    @unittest.skipIf(ATB_API_TOKEN is None, 'No ATB API token defined as ATB_API_TOKEN environmental variable')
    ...
    def test_molecule_query_bystructure(self):
        ...
        query_file = os.path.join(self._files_dir, '7233_querystructure.pdb')
        ...
        if result and isinstance(result, dict):
            self.assertTrue(any([m['molid'] == 7233 for m in result['matches']]))
    ...
    def test_host_token_not_valid(self):
        ...
        api = ATBServerApi(api_token='not_valid_token',
                           host=SETTINGS['atb_url'])

        valid_atb_token = True
        try:
            api.Molecules.search(any='ethanol')
        except (HTTPError, URLError):
            valid_atb_token = False

        self.assertFalse(valid_atb_token)
    ...
    def test_molecules_topoloy_lammps_save(self):
        ...
        molecule = self.api.Molecules.molid(molid=7233)
        for fformat in ('lammps_allatom_optimised.lt', 'lammps_allatom_unoptimised.lt',
            ...
            pdb_path = os.path.join(self._files_dir, '{0}_{1}_{2}.{3}'.format(molecule.molid, fformat.split('.')[0],
                                                                              '54A7', fformat.split('.')[1]))
            self.files_to_delete.append(pdb_path)

            mol = molecule.download_file(file=fformat, outputType='top', ffVersion='54A7', hash='LATEST')
            with open(pdb_path, 'w') as outf:
                outf.write(mol)
    ...
    def test_molecules_topoloy_gromos96_save(self):
        ...
        for fformat in ('mtb96_allatom', 'mtb96_uniatom'):
            pdb_path = os.path.join(self._files_dir, '{0}_{1}_{2}.{3}'.format(molecule.molid, fformat, '54A7',
                                                                              fformat.split('_')[1]))
            self.files_to_delete.append(pdb_path)
    ...
    def test_molecules_structure_gromacs_save(self):
        ...
        for fformat in ('rtp_allatom', 'rtp_uniatom'):
            pdb_path = os.path.join(self._files_dir, '{0}_{1}_{2}.itp'.format(molecule.molid, fformat, '54A7'))
            self.files_to_delete.append(pdb_path)
    ...
    def test_molecules_structure_pdb_save(self):
        ...
        for fformat in ('pdb_allatom_optimised', 'pdb_allatom_unoptimised', 'pdb_uniatom_optimised',
            ...
            pdb_path = os.path.join(self._files_dir, '{0}_{1}_{2}.pdb'.format(molecule.molid, fformat, '54A7'))
            self.files_to_delete.append(pdb_path)
    ...
    def test_molecules_structure_apbs_save(self):
        ...
        for fformat in ('pqr_allatom_optimised', 'pqr_allatom_unoptimised', 'pqr_uniatom_optimised',
            ...
            pdb_path = os.path.join(self._files_dir, '{0}_{1}_{2}.pqr'.format(molecule.molid, fformat, '54A7'))
            self.files_to_delete.append(pdb_path)

            mol = molecule.download_file(file=fformat, outputType='cry', ffVersion='54A7', hash='LATEST')
            with open(pdb_path, 'w') as outf:
                outf.write(mol)
    ...
    def test_molecules_structure_cns_save(self):
        ...
        for fformat in ('cns_allatom_top', 'cns_allatom_param', 'cns_uniatom_top', 'cns_uniatom_param'):
            pdb_path = os.path.join(self._files_dir, '{0}_{1}_{2}.cns'.format(molecule.molid, fformat, '54A7'))
            self.files_to_delete.append(pdb_path)
    ...
    def test_molecules_structure_cif_save(self):
        ...
        for fformat in ('cif_allatom', 'cif_allatom_extended', 'cif_uniatom', 'cif_uniatom_extended'):
            pdb_path = os.path.join(self._files_dir, '{0}_{1}_{2}.cif'.format(molecule.molid, fformat, '54A7'))
            self.files_to_delete.append(pdb_path)
    ...
    def test_submit_job_allreadycalculated(self):
        ...
        mol = molecule.download_file(file='pdb_allatom_optimised', outputType='top', ffVersion='54A7', hash='LATEST')
        try:
            response = self.api.Molecules.submit(pdb=mol, netcharge=0, moltype='heteromolecule', public=True)
        except HTTPError as error:
            response = str(error)

        if response.startswith('Your submission matched a previously'):
            m = re.search('(?<=molid=)[0-9]*', response)
            if m:
                molid = m.group()
                if molid.isdigit():
                    molid = int(molid)
